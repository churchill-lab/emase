...
class Sparse3DMatrix:
    ...
    def __init__(
        self, other=None, h5file=None, datanode='/', shape=None, dtype=float
    ):
        self.shape = (0, 0, 0)
        self.ndim = 3
        self.data = list()
    ...
    def __add__(self, other):
        if self.finalized:
            ...
            if isinstance(other, Sparse3DMatrix):
                ...
                other, (csc_matrix, csr_matrix, coo_matrix, lil_matrix)
            ):
    ...
    def sum(self, axis=2):
        if self.finalized:
            if axis == 0:
                ...
                sum_mat = np.vstack(
                    sum_mat
                )  # TODO: Fix the dimension by using hstack?
            else:
        ...
        if self.finalized:
            if multiplier.ndim == 1:
                if axis == 0:  # multiplier is np.array of length |haplotypes|
                    ...
                    for hid in range(self.shape[1]):
                        self.data[hid].data *= multiplier[
                            self.data[hid].indices
                        ]
                else:
                    raise RuntimeError('The axis should be 0, 1, or 2.')
                ...
                for hid in range(self.shape[1]):
                    self.data[hid] = self.data[hid].multiply(
                        multiplier.data[hid]
                    )
            else:
                raise RuntimeError(
